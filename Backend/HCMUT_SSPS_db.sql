...
CREATE TABLE IF NOT EXISTS file_types ( -- list all accpeted file types in the system (managed by SPSO)
	id				INT					AUTO_INCREMENT			PRIMARY KEY,
    type            VARCHAR(10)  								UNIQUE , -- .pdf, .docx,...
...
CREATE TABLE IF NOT EXISTS files ( -- Weak entity of Students, cause one file can be uploaded by multiple students on their private repository
	id				INT				AUTO_INCREMENT				PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS papers ( -- all existing paper types information (managed by SPSO)
	...
	FOREIGN KEY (spso_id) REFERENCES users(id) ON DELETE CASCADE -- if SPSO is deleted, all paper types added by that SPSO will be seemed to be default paper types
);
